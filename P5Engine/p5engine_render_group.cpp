	...
	loaded_bitmap* Texture,	real32 PixelsToMeters)
{
	...
	for (int Y = YMin; Y <= YMax; ++Y)
	{
		...
		// TODO: Remove this
		__m128 PixelPosX = _mm_set_ps((real32)(XMin + 3),
									  (real32)(XMin + 2),
									  (real32)(XMin + 1),
									  (real32)(XMin + 0));
...
#define mmSquareRoot(a) _mm_sqrt_ps(a)
...
#define _mm_storeu_si128(a, b) 0
#endif
			
			__m128 U = _mm_add_ps(_mm_mul_ps(PixelPosX, nXAxisx_4x), _mm_mul_ps(PixelPosY, nXAxisy_4x));
			__m128 V = _mm_add_ps(_mm_mul_ps(PixelPosX, nYAxisx_4x), _mm_mul_ps(PixelPosY, nYAxisy_4x));
			...
			// if (_mm_movemask_epi8(WriteMask))
			{
				...
				Blendedr = _mm_mul_ps(One255_4x, mmSquareRoot(Blendedr));
				Blendedg = _mm_mul_ps(One255_4x, mmSquareRoot(Blendedg));
		...
		switch (Header->Type)
		{
			...
			case render_group_entry_type::render_entry_bitmap:
			{
				...
				DrawRectangleQuickly(
					...
					Basis.Scale * V2(Entry->Size.x, 0), 
					Basis.Scale * V2(0, Entry->Size.y), 
					Entry->Color, Entry->Bitmap, PixelsToMeters
				);
#endif
