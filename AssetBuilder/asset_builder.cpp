...
#else
	entire_file TTFFile = ReadEntireFile((char*)Filename);
...
internal bitmap_id
AddCharacterAsset(game_assets* Assets, char* FontFile, char* FontName, u32 Codepoint, f32 AlignPercentageX = 0.5f, f32 AlignPercentageY = 0.5f)
{
	Assert(Assets->DEBUGAssetType);
	Assert(Assets->DEBUGAssetType->OnePastLastAssetIndex < ArrayCount(Assets->Assets));
...
WriteNonHero(void)
{
	...
	for (u32 Character = 'A';
		...
		++Character)
	{
		AddCharacterAsset(Assets, (char*)"c:/Windows/Fonts/arial.ttf", (char*)"Arial", Character);
